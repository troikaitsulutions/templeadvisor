<?php
	...
	$js_ret = 'jQuery("#GetMyFilter").click(function() {
	   ...
	   $("#events-rituals :checked").each(function() {
		 allVals.push($(this).val());
	   });
	   
	   if(allVals != "") { params +="&beliefs="+allVals; }
	   window.location.replace("'.Yii::app()->createUrl('temples/bythemebeliefs').'"+params);
	});';
    ...
    <div class="theme-part">
      <form method="POST">
        <div class="theme-part-left">
          ...
          <div class="events-rituals collapse">
            <div class="tmp_search">
            <!--  <input id="search_box" type="text" value="<?php echo (isset($_GET['q']))?$_GET['q']:''; ?>"/> -->
            </div>
            <div id="events-rituals">
              <h4><?php echo t('Events/Rituals/Beliefs'); ?> </h4>
              <ul>
                <?php		
	        ...
	        $BeliefList = Belief::model()->findAll(array("condition"=>"status = 1","order"=>"TRIM(name) ASC"));	
			if( isset($BeliefList) && count($BeliefList)>0 ) {
				foreach ( $BeliefList as $BeliefName ) {
					
					$tot_temples = Temples::model()->findAll(array('condition'=>'status = 1 AND religion = 1001 AND belief like "%'.$BeliefName->id.'%"'));
					
					if( isset($tot_temples) && count($tot_temples) >= 10 ) {
						
						$ListCheck = '<li><label> <input name="deitybox" type="checkbox"';
								if(in_array($BeliefName->id, $adids)) { $ListCheck .=" checked "; }
								$ListCheck .= ' value="'.$BeliefName->id.'"> '.$BeliefName->name.'('.count($tot_temples).')</label></li>';
								echo $ListCheck;
          ...
          <div id="horizontalTab">
            <div class="resp-tabs-container">
              ...
              <div class="resp-tab-content resp-tab-content-active" aria-labelledby="tab_item-0" style="display:block">
                <div class="temple-menu-count">
                  <div class="temple-menu-rr">
                    <ul>
                      ...
                      <li><a href="<?php echo Yii::app()->createUrl('temples/bythemehistory'); ?>"><?php echo t('History'); ?></a></li>
                      <li><a href="<?php echo Yii::app()->createUrl('temples/bythemebeliefs'); ?>" class="temple-menu-rr-active"><?php echo t('Beliefs'); ?></a></li>
                ...
                <div class="temple-list-count"><?php echo t('Total No of Temples :'); ?> <span id="total_temples"><?php echo $item_count; ?></span></div>
                <div class="theme-god-row" >
                  <ul id="projects" class="pagination1">
