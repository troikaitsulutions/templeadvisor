				...
				if( isset($DietyList) && count($DietyList)>0 ) {
					foreach ( $DietyList as $DietyName ) {
						...
						if( isset($tot_temples) && count($tot_temples) >= 0 ) {
								...
								if(in_array($DietyName->id, $adids)) { $ListCheck .=" checked "; }
								$ListCheck .= ' value="'.$DietyName->id.'"> '.$DietyName->name.'('.count($tot_temples).')</label></li>';
								echo $ListCheck;
					} } }
				?>
              </ul>
            </div>
           <div class="list-by-all-butrr"> <button type="button" name="getmylist" class="getmylistbt" id="GetMyFilter" > Get My List </button> </div>
          </div>
        </div>
      </form>
