...
$csrf_token = Yii::app()->request->csrfToken;
    $str_js = "
        ...
        $('#ovgallery-grid table.items tbody').sortable({
            forcePlaceholderSize: true,
            forceHelperSize: true,
            items: 'tr',
            update : function () {
                serial = $('#ovgallery-grid table.items tbody').sortable('serialize', {key: 'items[]', attribute: 'class'})+ '&{$csrf_token_name}={$csrf_token}';
                $.ajax({
                    'url': '".Yii::app()->request->baseUrl."/beovgallery/sort',
                    'type': 'post',
                    'data': serial,
	...
	$('#gal_del_all').click(function(){
		...
		if(idList)
		{
			if(confirm('Are you sure want to delete?'))
				{
					$.get('".Yii::app()->request->baseUrl."/beovgallery/deleteall',idList,function(response){
	   ...
	   /* array
		(
			...
			(
			   'label'=>t('Translate'),
			    'imageUrl'=>false,
			    'url'=>'Yii::app()->createUrl("'.app()->controller->id.'/update", array("guid"=>$data->guid,"page_id"=>$data->prop_id))',
			),
		    ),
		),
		 */   			
