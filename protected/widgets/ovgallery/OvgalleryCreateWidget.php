 ...
 *
 */
class OvgalleryCreateWidget extends CWidget
{
    ...
    {       
		...
		if (!empty($_FILES)) {
			...
			$file = CUploadedFile::getInstanceByName('Ovgallery[file]');
			$temp_name = toSlug(Overview::GetName($page_id)).'_'.time().'.'.$file->getExtensionName();
			...
			if (Yii::app()->s3->putObjectFile(RESOURCES_FOLDER.'/'.$temp_name, AWS_S3_BUCKET, $page_id.'/large/'.$temp_name, S3::ACL_PUBLIC_READ, $metaHeaders = array(
				...
				{  
					...
					$img_no = Ovgallery::model()->find(array(
        ...
        $this->render('cmswidgets.views.ovgallery.ovgallery_form_widget',array('model'=>$model));
    }   
}
