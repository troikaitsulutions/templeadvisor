...
class PinfoImportWidget extends CWidget
{
    ...
    {       
		...
		'condition'=>'IDPROPERTY=0 AND PUBLICWS = 0 AND SITO2=1',
		'order'=>'ID ASC'));	
					
					$Season = Period::model()->findAll(array(
					'condition'=>'IDPROPERTY ='.$model->old_id.' AND ACT_STATUS = 1',
    				'order'=>'ID',
					)); 
					...
					if($Season) {
						foreach ($Season as $s)
						{
							...
							$NewSeason->prop_id = $model->primaryKey;
							$NewSeason->name = $s->PERIOD_NAME;
							$NewSeason->created = $s->LASTUPDATE;
							$NewSeason->lang = 2;
				 			$NewSeason->cr_ip = ip();
							$NewSeason->save();
							...
							if($s->FROM1)
							{
								...
								$sdate->season_id = $NewSeason->primaryKey;
								$sdate->from_date = $s->FROM1;
								$sdate->to_date = $s->TO1;
								$sdate->created = $s->LASTUPDATE;
				 				$sdate->cr_ip = ip();
								$sdate->save();
							...
							$Tar = Tarrif::model()->findAll(array(
							'condition'=>'IDPERIOD ='.$s->ID,
    						'order'=>'ID',
							));
							...
							if($Tar) {
								foreach ($Tar as $t){
									...
									$R->season_id = $NewSeason->primaryKey;
									$R->people = $t->PAXMAX;
									$R->price_week = $t->PRICEWEEK;
									$R->price_day = $t->PRICEDAY;
									$R->created = $t->LASTUPDATE;
				 					$R->cr_ip = ip();
									$R->save();
								}
							}
