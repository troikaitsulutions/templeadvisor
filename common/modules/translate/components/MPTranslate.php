...
class MPTranslate extends CApplicationComponent{
     ...
     * */
    public $languageKey='mp-translate';
    /**
    ...
    function translateLink($label='Translate',$type='link'){
        //$form=CHtml::form(Yii::app()->createUrl(Yii::app()->getBaseUrl().'translate/translate/index'));
	$form=CHtml::form(Yii::app()->createUrl('/translate/translate/index'));
        if(count(self::$messages))
            foreach(self::$messages as $index=>$message)
                foreach($message as $name=>$value)
                    $form.=CHtml::hiddenField($this->languageKey."-missing[$index][$name]",$value);
        if($type==='button')
    ...
    function translateDialogLink($label='Translate',$title=null,$type='link'){
        return $this->ajaxDialog($label,'translate/translate/index',$title,$type,array('data'=>array($this->languageKey.'-missing'=>self::$messages)));
    }
    /**
    ...
    private function ajaxDialog($label,$url,$title=null,$type='link',$ajaxOptions=array()){
        ...
        $url=Yii::app()->createUrl(Yii::app()->getBaseUrl().$url);
