...
class tlbExcelView extends CGridView
{
    ...
    public function init()
    {
        ...
        echo $lib = Yii::getPathOfAlias($this->libPath).'.php';
        ...
        if ($this->grid_mode == 'export') {
			...
			//self::$objPHPExcel->getActiveSheet()->getStyle()->getFont()->setSize(20);
			//$styleArray = array('font' => array('size' => 20));
			//self::$objPHPExcel->getActiveSheet()->duplicateStyleArray($styleArray, 'A1:A100');
    ...
    public function renderHeader()
    {
        ...
        $count = count($datas);
		$cell = self::$activeSheet->setCellValue('A1', 'Total Rows : ', true);
		$cell = self::$activeSheet->setCellValue('B1', $count, true);
		$summary = self::$activeSheet->getStyle('A1:B1');
        $summary->getAlignment()
            ->setHorizontal(self::$horizontal_center)
            ->setVertical(self::$vertical_center);
        $summary->applyFromArray(array(
                'borders' => array(
                    'allborders' => array(
                                        'style' => $this->border_style,
                                        'color' => array('rgb' => $this->headerBorderColor),
        ...
        foreach ($this->columns as $column) {
            ...
            $cell = self::$activeSheet->setCellValue($this->columnName($a) . '2', $head, true);
        ...
        $header = self::$activeSheet->getStyle($this->columnName(1) . '2:' . $this->columnName($a) . '2');
        $header->getAlignment()
            ->setHorizontal(self::$horizontal_center)
            ->setVertical(self::$vertical_center);
        $header->applyFromArray(self::$headerStyle);
        self::$activeSheet->getRowDimension(2)->setRowHeight($this->headerHeight);
    }
    ...
    public function renderRow($row)
    {
        ...
        foreach ($this->columns as $n => $column) {
            ...
            $cell = self::$activeSheet->setCellValue($this->columnName($a) . ($row + 3), $content, true);

            // Format each cell's number - if any
            if (!is_null($format)) {
                self::$summableColumns[$a] = $a;
                self::$activeSheet->getStyle($this->columnName($a) . ($row + 3))->getNumberFormat()->setFormatCode($format);
            }
        ...
        $renderedRow = self::$activeSheet->getStyle('A' . ($row + 3) . ':' . $this->columnName($a) . ($row + 3));
        $renderedRow->getAlignment()->setVertical(self::$vertical_center);
        $renderedRow->applyFromArray(self::$style);
        self::$activeSheet->getRowDimension($row + 3)->setRowHeight($this->rowHeight);
    }
