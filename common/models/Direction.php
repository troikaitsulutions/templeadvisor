...
class Direction extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('prop_id, vehicle, town, travel_time', 'required'),
			array('prop_id, town, vehicle', 'numerical', 'integerOnly'=>true),
			array('name, travel_time', 'length', 'max'=>255),
            //array('name','unique'),     
           // array('prop_id+name+year', 'common.extensions.uniqueMultiColumnValidator'),             
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, prop_id, name, status, vehicle, town, travel_time', 'safe'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => t('ID'),
			'name' => t('Vehicle Name/Number'),
			'vehicle' => t('Vehicle'),
			'lang' => t('Language'),
            'town' => t('Town From'),
			'travel_time' => t('Travel Time')
		);
	}
