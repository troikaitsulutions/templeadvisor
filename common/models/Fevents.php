 ...
 */
class Fevents extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('status, crby, mod_by, created, modified, addtohome', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>255),
			array('h3', 'length', 'max'=>60),
			array('h3_desc', 'length', 'max'=>1000),
			array('content', 'length', 'min'=>10),
			array('cr_ip, mod_ip','safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, h3, h3_desc, uid', 'safe', 'on'=>'search'),
		);
	}
