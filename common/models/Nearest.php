 ...
 */
class Nearest extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('prop_id, things, town, distant', 'required'),
			array('prop_id, town, things', 'numerical', 'integerOnly'=>true),
			array('name, distant', 'length', 'max'=>255),
            //array('name','unique'),     
           // array('prop_id+name+year', 'common.extensions.uniqueMultiColumnValidator'),             
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, prop_id, name, status, things, town, distant', 'safe'),
		);
	}
		...
		// class name for the relations automatically generated below.
		 return array(                    
            'language' => array(self::BELONGS_TO, 'Language', 'lang'),
             'Things' => array(self::BELONGS_TO, 'Nthings', 'things'),
	...
	public function searchByobject($object_id)
	{
		...
		//$criteria->compare('id',$this->id,true);
		$criteria->compare('name',$this->name,true);
		$criteria->compare('prop_id',$object_id);
