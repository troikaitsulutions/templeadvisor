...
class Pujalogistic extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('status, name, price, add_price', 'required'),
			array('id, crby, created, modified, mod_by, status', 'numerical', 'integerOnly'=>true),
			array('price, add_price', 'numerical', 'integerOnly' => false, 'min' => 1),
			array('name, cr_ip, mod_ip', 'length', 'max'=>255),
			array('id, name, price, add_price', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'name' => t('Country Name'),
			'status'=> t('Status'),
			'price' => t('Price(upto 250 Gms)'),
			'add_price' => t('Price(additional 250 Gms)'),
		);
	}
