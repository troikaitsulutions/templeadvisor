...
class MenuItem extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('menu_id, name, page', 'required'),
			array('menu_id, parent, page', 'numerical', 'integerOnly'=>true),
			array('name, type, description', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('menu_item_id, menu_id, name, page', 'safe', 'on'=>'search'),
		);
	}
        ...
        protected function beforeSave(){
            if(parent::beforeSave())
            {
                    ...
                    {				
                            ...
                            ->createCommand("SELECT MAX(`order`)+1 FROM tt_menu_item where menu_id = :tid and parent = :pid ")
            ...
            if(Yii::app()->request->isPostRequest){       
                    if( isset($_POST['data']) && (strpos($_POST['data'],"li_item_id[]")!==false)){                        
                        ...
                        foreach($order as $order=>$id){
                            ...
                            $command->update('tt_menu_item', array(
                                'order'=>$order+1,
