...
class GoogleCalendarProxy {
	...
	public function __construct($email, $pass, $cal_name) {
		...
		$this->auth='DQAAAMMAAABL-Q3t8RQxuwBb9pV-TYZmeB1-ElSIhTWj55Q_GUIfmhsXnkp77pYja5vNEZh9gBXhYUhGOpjehUce9FqB_pCmki5r2tgBaWIlil-1EcCgSnpQZN67bjU4v--c03MXzb-f_lTuRpUZMdRoHLK7StAnxETq98jZZZleseq-Bpm2ZCTlH1EcTWlYGEM-B9Dm-YV5m9rKXJH9EQCwqNjmUyhst__oLMguAEo6y8NZpbIDt9-Tw1SmCoMHrQf-iqYTOCRcia4sGPOqS0Xm-U9zEgLB';
	...
	"data": {
		...
		"when": [
			{
				"start": "2013-11-17T15:00:00.000Z",
				"end": "2013-11-17T17:00:00.000Z"
			}
		]
	}
}';
...
//		$msg = $delim.date("Y-m-d H:i:s").$delim.$msg.$delim;
	 ...
	 */
	public function export($connection, $table, $prop_id) {
		$this->log($this->delim.date("Y-m-d H:i:s").$this->delim."Retrieving calendars");
		...
		if (isset($this->cals_hash[$this->cal_name])) {
			...
			for ($i = 0; $i < count($events); $i++) {
				...
				foreach ($this->export_map as $googleField => $dhtmlxField) {
					if (!isset($ev->$googleField)) continue;
					$fields[] = $dhtmlxField;
					$values[] = strtotime($ev->$googleField);
				}
				$booking_id = 'CIAO-'.$prop_id.'-ICAL-'.date('ymdhis').$i;
				$created = time();
				$cr_ip = ip();
				$crby = Yii::app()->user->getId();
				$query = '';
				if($prop_id!=''){
				$values[1] = strtotime('-1 day',$values[1]);
				$check = "select * from ".$table." where date_add(DATE_FORMAT(FROM_UNIXTIME(`fdate`),'%Y-%m-%d'), interval 1 day)='".date('Y-m-d',$values[0])."' and date_add(DATE_FORMAT(FROM_UNIXTIME(`tdate`),'%Y-%m-%d'), interval 1 day)='".date('Y-m-d',$values[1])."' and prop_id='$prop_id'";
				$rs = mysql_query($check);
				$rscount = mysql_num_rows($rs);
				if($rscount==0)
				{
					$query[$i] = "INSERT INTO ".$table." (booking_id,prop_id,customer_id,fdate,tdate,bsource,confirm,status,created,crby,cr_ip) VALUES ('$booking_id','$prop_id','1','".$values[0]."','".$values[1]."','6','1','1','$created','$crby','$cr_ip');";
					mysql_query($query[$i]);
				}}
	 ...
	 */
	public function import($connection, $table, $prop_id) {
		$this->log($this->delim.date("Y-m-d H:i:s").$this->delim."Retrieving calendars");
		...
		if (isset($this->cals_hash[$this->cal_name])) {
			...
			$query = "SELECT date_add(DATE_FORMAT(FROM_UNIXTIME(`fdate`),'%Y-%m-%d 00:00:00'), interval 1 day) as start_date,date_add(DATE_FORMAT(FROM_UNIXTIME(`tdate`),'%Y-%m-%d 00:00:00'), interval 2 day) as end_date,booking_id as event_name FROM ".$table." where prop_id='$prop_id'";
			...
			while ($event = mysql_fetch_assoc($res)) {
				...
				if(date('Y-m-d',$values[0])==date_format(date_create($event['start_date']),'Y-m-d') && date('Y-m-d',$values[1])===date_format(date_create($event['end_date']),'Y-m-d')) $k++;
				}
				if($k==0)
				$xml = $this->insertEvent($ev, $ev_json);
				$count++;
	...
	public function pushCalToSession() {
		$this->cals_hash[$this->cal_name] = $this->cal_name;
