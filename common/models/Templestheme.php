...
class Templestheme extends CActiveRecord
{
	/**
	...
	public function search()
	{
		...
		$ThemeList = Themelist::model()->findAll(array('condition'=>'source = :SID','params'=>array(':SID'=>$Themes->id)));
		...
		if ( (isset($_GET['themelist']) && ($_GET['themelist']!='' ) ) || (isset($_GET['region']) && ($_GET['region']!='' ) ) )
		{
		
		$criteria2 = new CDbCriteria;
		
		if( isset($_GET['themelist']) && ($_GET['themelist']!=''))
		{
			$pieces = explode(",", $_GET['themelist']);
			
			if(is_array($pieces)) {
				
				foreach ($pieces as $key => $value) 
					{ 
						$sql1[] = 'themelist LIKE "%'.$value.'%"';
						
					}
				$query1 = implode(" OR ", $sql1);
				$criteria2->condition = $query1;
			}
		}	
		
		if( isset($_GET['region']) && ($_GET['region']!=''))
		{
			$pieces2 = explode(",", $_GET['region']);
			
			if(is_array($pieces2)) {
				foreach ($pieces2 as $key => $value) 
					{ 
						$criteria2->compare('region', $value, false, 'OR');
					} 
			}
		}	
